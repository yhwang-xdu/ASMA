...
class CoreDetector(AbstractDetector):
    ...
    def features(self, data_dict: dict) -> torch.tensor:
        # Check if data_dict is a tensor or a dictionary
        if isinstance(data_dict, dict):
            # If it's a dictionary, extract the image tensor from it
            image = data_dict['image']
        else:
            # If it's already a tensor, use it directly
            image = data_dict
    
        # Now extract the features from the image tensor
        return self.backbone.features(image)
